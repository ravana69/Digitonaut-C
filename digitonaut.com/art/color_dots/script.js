...
function setup() {
  // loadImage('https://alca.sfo2.cdn.digitaloceanspaces.com/uploads/codepen/2019/04/26/f8661882-9611-45bd-94fe-1935a27002d4.png').then(i => img = i);
}
...
function draw(e) {
  ...
  if (particles.length < 1000) {
    ...
    const v = Vector.fa(random(TAU), random(200));
    p.pos.set(v);
    particles.push(p);
  }
  // !img && beginPath();
  const mp = mousePos._.sub(Vector.center());
  for (let i = particles.length - 1; i >= 0; i--) {
    ...
    for (let j = particles.length - 1; j >= 0; j--) {
      ...
      if (dist < rad) {
        ...
        // else if(p.r < 10) {
        p.vel.add(diff._.limit(0.01 * p2.r));
        p2.vel.add(diff._.limit(-0.01 * p.r));
        // }
        break;
      }
    }
    if (mouseDown && mp.dist(p.pos) < p.r) {
      // particles.splice(i, 1);
      p.r *= 0.97;
    }
    p.pos.add(p.vel.mult(0.99));
    if (canGrow) {
      p.r += 0.5;
    } else
    {
    ...
    if (!img) {
      beginPath();
      circle(p.pos, p.r);
      fill(hsl(p.color, 100, 50));
      // circle(p.pos, max(p.r - 5, 0));
      // globalAlpha(min(1, map(p.r, 0, 10, 0, 1)));
      // fill(hsl(p.color, 100, 50, 0.5));
      // stroke(hsl(p.color, 100, 50), 10);
    } else
    {
      const blurRadius = min(5, abs(40 - p.r) * 0.05);
      filter(`blur(${blurRadius}px) hue-rotate(${p.color}deg)`);
      // if(p.r < 100) {
      // }
      // else {
      // 	filter('none');
      // }
      const r = p.r + blurRadius;
      // drawImage(img, p.pos.x - p.r, p.pos.y - p.r, p.r * 2, p.r * 2);
      drawImage(img, ...p.pos._.sub(r).xy, r * 2, r * 2);
    }
    if (p.pos.mag() > 3000) {
      particles.splice(i, 1);
    }
  }
  // !img && stroke(hsl(0, 0, 100));
}

class Particle {constructor() {_defineProperty(this, "pos",
    new Vector());_defineProperty(this, "vel",
    new Vector());_defineProperty(this, "r",
    0);_defineProperty(this, "color",
    random(360));} //random(360 / 7) * 7 + 20;
}